...
def test_combat_bladder():
    ...
    effects_interest = pd.get_dummies(effects_interest, drop_first=True)
    ...
    Y_combat_w_effects = combat.fit_transform(in_features=in_features, 
                                              batch=batch, 
                                              effects_interest=effects_interest)
    assert np.allclose(Y_combat_w_effects, bladder_test['Y_combat_effects'])
